...
func (eq *EventQuery) QueryFights() *FightQuery {
	query := (&FightClient{config: eq.config}).Query()
...
func (eq *EventQuery) Clone() *EventQuery {
	...
	return &EventQuery{
		...
		withFights: eq.withFights.Clone(),
...
func (eq *EventQuery) WithFights(opts ...func(*FightQuery)) *EventQuery {
	query := (&FightClient{config: eq.config}).Query()
...
func (eq *EventQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Event, error) {
	...
	if query := eq.withFights; query != nil {
		if err := eq.loadFights(ctx, query, nodes,
			func(n *Event) { n.Edges.Fights = []*Fight{} },
			func(n *Event, e *Fight) { n.Edges.Fights = append(n.Edges.Fights, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (eq *EventQuery) loadFights(ctx context.Context, query *FightQuery, nodes []*Event, init func(*Event), assign func(*Event, *Fight)) error {
	...
	query.withFKs = true
	query.Where(predicate.Fight(func(s *sql.Selector) {
		s.Where(sql.InValues(event.FightsColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.event_fights
		if fk == nil {
			return fmt.Errorf(`foreign-key "event_fights" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "event_fights" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
